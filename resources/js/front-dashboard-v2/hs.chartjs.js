			...
			newChartJS.draw = function (ease) {
				...
				if (this.chart.tooltip._active && this.chart.tooltip._active.length) {
					...
					if (lineTooltip) {
						...
						lineTooltip.style.backgroundColor = settings.options.tooltips.lineWithLineColor
					...
					custom: function (tooltipModel) {					
						...
						if (tooltipModel.body) {
							...
							titleLines.forEach(title => {
								innerHtml += this._options.yearStamp ? title + ', ' + today.getFullYear() : title
							})
							...
							bodyLines.forEach((body, i) => {
								...
								innerHtml += (this._options.hasIndicator ? '<span class="d-inline-block rounded-circle me-1" style="width: ' + this._options.indicatorWidth + '; height: ' + this._options.indicatorHeight + '; background-color: ' + color + '"></span>' : '') + this._options.prefix + (oldBody.length > 3 ? newBody : body) + this._options.postfix
						...
						tooltipEl.style.transition = this._options.transition
					}
				}
			}
		}, true)
