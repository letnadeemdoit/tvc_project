...
class Select2Controller extends Controller
{
    public function houses()
    {
        return response()->json([
            'results' => House::selectRaw('HouseID as id, CONCAT(HouseName, " (", IF(Address1 is not null and Address1 <> "", CONCAT(Address1, ", "), ""), IF(Address2 is not null and Address2 <> "", CONCAT(Address2, ", "), ""), IF(City is not null and City <> "", CONCAT(City, ", "), ""), IF(State is not null and State <> "", CONCAT(State, ", "), ""), IF(Country is not null and Country <> "", CONCAT(Country, ""), ""), ")") as text')
